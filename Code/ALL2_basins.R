...
path_file <- 'Data/'
path_big_file <- '~/Library/CloudStorage/OneDrive-KarolinskaInstitutet/Protein_landscape/ALL_cell_lines/'
path_save <- 'Figures/'
...
all_samples_basins_neg <- get.basins(sampleIDs = colnames(proteomics_quant), pickle_dat = pickle_data_neg, basin_length_thres_low = 2, basin_length_thres_high = nrow(proteomics_quant), sign_level = 'negative')
...
# all_samples_overlap <- all_samples_overlap[all_samples_overlap$cor >= 0.9, ]
#
# # Maximum cliques
...
top_scores <- do.call(rbind, lapply(1:nrow(scaled_profiles), function(i) {
  ...
  #
  res_uni_all <- sapply(1:K, function(j) {
...
h_membership$class <- factor(h_membership$class, levels = paste0('NMF', 1:K))
...
#
# pdf(file = paste0(path_save, 'ALL_cell_line_NMFs_heatmap_non_outliers.pdf'), width = 15, height = 15)
...
pdf(file = paste0(path_save, 'ALL_cell_line_NMFs_heatmap_non_outliers_enriched.pdf'), width = 12, height = 12)
draw(ht_all, padding = unit(c(2, 20, 2, 2), "mm"))
dev.off()
